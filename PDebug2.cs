...
namespace PDebug { 
    ...
    public class PDebugInit{
        [RuntimeInitializeOnLoadMethod]
        public static void OnRuntimeMethodLoad(){
            Camera.main.gameObject.AddComponent<PDebug.PDebug2>();
        }
    }
    ...
    public class PDebug2 : MonoBehaviour{
        ...
        private static List<CustomDll> loadedCustomDlls = new List<CustomDll>();

        private class CustomDll{

            public string filePath;
            public Assembly assembly;

            public CustomDll(string filePath, Assembly assembly){
        ...
        private string typeInput = "PDebugCustomDLLTemplate.CustomBehaviour";
        private void DrawCustomDll() {
            ...
            this.dllInput = GUI.TextField(new Rect(10f, 65f, 350, 18), this.dllInput);
            this.typeInput = GUI.TextField(new Rect(10f, 85f, 350, 18), this.typeInput);
        ...
        public void AddTypeFromAssembly(string dllPath, string typeName, GameObject go){
            ...
            if (!File.Exists(dllPath)){
                Debug.Log("Can't find " + dllPath + "!");
                return;
            }

            Assembly assembly = null;
            foreach (CustomDll dll in PDebug2.loadedCustomDlls){
                if (dll.filePath == dllPath){
                    assembly = dll.assembly;
                    Debug.Log("CustomDll was already injected, try to add Type from loaded Dll.");
                }
            }

            if (assembly == null){
                assembly = Assembly.LoadFrom(dllPath);
                PDebug2.loadedCustomDlls.Add(new CustomDll(dllPath, assembly));
            }

            System.Type testBehaviour = assembly.GetType(typeName);
